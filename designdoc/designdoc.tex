...
\begin{figure}
	\centering
	\begin{tikzpicture}
		\node (se1) at (0,0) [draw, rectangle,minimum width=9cm, minimum height=2cm] {State estimation};
		...
		\node (pos) at (0, 3) [draw=none] {Acceleration};
		\node (v) at (2, 3) [draw=none] {GPS};
		\node (son2) at (4, 3) [draw=none] {Sonar};
		...
		\draw[triangle 60-] (son2) to +(0,-2);
		
	\end{tikzpicture}
	\caption{Interfaces of the "state estimation" block}
	\label{fig:state_est}
\end{figure}
\begin{figure}
	\centering
	\begin{tikzpicture}
	\node (se1) at (0,0) [draw, rectangle,minimum width=9cm, minimum height=2cm] {Localization};
	\node (att2) at (-3.5, 3) [draw=none] {Attidute};
	\node (rot2) at (-1.25, 3) [draw=none] {Rotation};
	\node (pos) at (1.25, 3) [draw=none] {Position};
	\node (v) at (3.5, 3) [draw=none] {Velocity};
	\node (pose) at (7, 0) [draw=none] {Position};
	
	\node (att) at (-4, -3) [draw=none] {Attitude};
	\node (rot) at (-2, -3) [draw=none] {Rotation};
	\node (acc) at (0, -3) [draw=none] {Acceleration};
	\node (gps) at (2, -3) [draw=none] {GPS};
	...
	\end{tikzpicture}
	\caption{Interfaces of the "localization" block}
	\label{fig:state_est2}
\end{figure}
The block "state estimation" will be implemented as a Kalman-filter and have the interfaces as shown in figure (\ref{fig:state_est})
\FloatBarrier
